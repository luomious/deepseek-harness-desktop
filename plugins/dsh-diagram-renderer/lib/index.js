...
export function apply(ctx) {
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      scene: { type: 'object', additionalProperties: true, description: '应用场景/系统架构类内容**必传**（车间/产线/应用层架构、信号流、数据流）。v9 结构 { title, subtitle?, theme?, actors:[{ id?, name, desc?, type?, icon?, highlight? }], groups?: [{ id?, name, members:[actorId], color? }], flows:[{ from, to, label?, kind? }], footer?, showFooter? }。type 取 frontend/backend/data/cloud/security/bus/external/person/device/core——决定语义配色+图标（缺省按 icon 推断，旧 icon 库全兼容）；flow kind 取 data/sensor/control/event/security——决定线型与箭头色；theme 取 auto（默认·深浅自适应）/light/dark；groups 表达层/边界/泳道分区（未入组节点自动成末组）。智能排版：标题 17→13 缩放不截断、卡名 13→11 自适应、描述换行 ≤2 行、卡高随内容、行高取行内最大。footer 要点面板默认不渲染，需显式 showFooter:true（≤6 条）。' },
