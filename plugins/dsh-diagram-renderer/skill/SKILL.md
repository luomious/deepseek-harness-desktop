...
「应用场景」「整体方案」「系统组成」「产线布置」「系统架构」这类**若干实体 + 关系**的图 —— **只传数据**，v9 渲染内核自动完成：语义配色、分组分区、智能字号、自动布局（三向）+ 视觉身份（四套）+ 连线路由、深浅双主题。

**布局选型**：

| 结构特征 | layout | 说明 |
|---|---|---|
| 分层架构（默认） | `vertical` | 组纵排、组内横排（默认兜底） |
| 流程/管道（组间推进） | `horizontal` | 左→右泳道，每组一列 |
| 中心服务+周边依赖 | `radial` | hub 居中，辐条上下环绕 |
| 不确定 | `auto` | 引擎按拓扑推断（保守，无强特征退回 vertical） |

**视觉身份选型**：

| 场景气质 | preset | 说明 |
|---|---|---|
| 通用（默认） | `paper` | 暖纸白卡描边+色条 |
| 基础设施/工程 | `blueprint` | 深蓝图纸底+双线框+mono 角标 |
| 文档/方案/叙述 | `editorial` | 米白无底卡+底部分隔线+序号 |
| 高密度数据流 | `signal` | 冷白实底粗边+高对比 |
| 不确定 | `auto` | 引擎按类型分布推断 |
...
- `layout`：`auto`（默认，引擎按拓扑推断）/ `vertical`（分层）/ `horizontal`（泳道，流程/管道）/ `radial`（辐射，中心服务）
- `preset`：`auto`（默认，引擎按类型分布推断）/ `paper`（暖纸描边）/ `blueprint`（深蓝图纸）/ `editorial`（杂志编排）/ `signal`（信号流）
