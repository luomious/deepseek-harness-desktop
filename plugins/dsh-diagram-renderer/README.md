...
- 2026-09-06 **v6 分步交互图（stages）**：回应「WorkBuddy 示意图有上一步/下一步/播放，不够智能」——host `render_diagram` 新增可选 `stages` 数组参数（`{id,title,description?,layers?}`，normalize 上限 16 步），编码进信封 meta；client 新增 `StageViewer` 组件（与 DiagramViewer 并列，有 stages 时自动启用）：工具栏 = ◀ ▶ 播放（2s/步自动播，手动导航即停）· 阶段点（可点击跳转）· 标题 · 阶段 n/N · 全屏 · ⋮；图内左下角阶段标题/说明浮层（纸面毛玻璃）；按 `active.layers` 切换 `<g data-stage>` 显隐（`all` 常显），320ms 上浮渐入 + 400ms 高光过渡（v6.1）；键盘 ← → 切换、空格播放。SVG 层显隐经 DOMParser 序列化实现，**同一张卡完成多阶段叙事**。管线回归新增 stages 断言 3 条（正常/转义信封）→ 18/18 PASS；实调 `render_diagram` 生成「动态特征三级别剔除交互演示」（1000×540，4 阶段）落盘 `diagrams/`。**生效：client 刷新即活；host 需重启一次（loader.internal 不可用）。**
- 2026-09-06 **v6.1 动态效果**：阶段切换 320ms 上浮渐入 + 400ms 亮度高光（全局 keyframes 幂等注入 `#dsh-diagram-stage-styles`，`prefers-reduced-motion` 降级，动画容器 `.dsh-anim`）、自动播放控制条底部 2s 阶段进度条、播放中当前阶段点靛蓝脉冲。SKILL 动效章节升级：5 个即用模板（流动/脉冲/描线生长/数字跳动/闪烁徽章）+「分步图层重显会重放动画」配合说明。**🐛 修复 v6 阶段层切换从未生效**：Chrome `image/svg+xml` DOMParser 产出的 SVGElement 无 `.style`（HTMLElement 专属），`gs[i].style.display` 抛 TypeError 被 catch 吞掉静默返回原图（每步显示全图层）；改 `setAttribute('style')` 属性级手术（保留其余 CSS）。管线 18→21 断言（新增「确有过滤发生」断言——教训：静默早退直通会让直通断言掩盖真实路径失效）。