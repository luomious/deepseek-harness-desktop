...
export function apply(ctx: Context): void {
  ...
  const onBeforeUnload = (event: BeforeUnloadEvent): void => {
    const running = document.querySelector('button[aria-label="停止生成"], button[aria-label="Stop generating"]') !== null
    if (!running) return
    event.preventDefault()
    event.returnValue = ''
  }
  window.addEventListener('beforeunload', onBeforeUnload)
  ctx.effect(() => () => window.removeEventListener('beforeunload', onBeforeUnload), 'ui-skin-maid-atelier: beforeunload guard')
