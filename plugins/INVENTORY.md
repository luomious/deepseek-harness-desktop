...
> **计数纪律（2026-09-12 修正）**：本标题数字必须与实测一致 —— 复算：
> `(Get-ChildItem plugins -Directory | Where-Object { Test-Path (Join-Path $_.FullName 'package.json') }).Count`
> 或看运行态 `GET http://127.0.0.1:43120/health` 的 `plugins` 探测（2026-09-12 实测两端均为 **35**，且 35/35 都含 `lib/index.js`）。
> 历史：本标题曾长期写「**31 个**」，而**表格行数早已是 35**（台账标题滞后于台账内容，O9 同族）⇒ 已按实测改正。
...
- 总计: 34（plugins/ 31 + 根级 3）| core: 30 | experimental: 3 | deprecated: 1（dsh-vision-rotator）｜ profile 市场安装: 3（dsh-context + dsh-better-sidebar-plugin-office + dsh-memory-plugin）
- 装配方式（plugins/）：bundle 23 | patch-insert 8（根级守护另列）①
