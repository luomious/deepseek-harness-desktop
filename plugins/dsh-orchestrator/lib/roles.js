...
/**
 * 测试角色的 deny 清单（2026-09-14 e2e 走查发现的真问题）：
 * 测试角色**必须能执行命令**（跑测试、复现"改前失败/改后通过"的证据），所以放行 shell/terminal/pwsh；
 * 但仍禁文件写入与安装/补丁类工具。工作区完整性由 G5 负责（对 dev 声称改动过的文件做前后 hash），
 * 因为 shell 本身就能写文件——工具级禁用对它既不必要也不充分，G5 才是可验证的那道闸。
 */
const TEST_DENY = WRITE_TOOLS.filter((t) => !['shell', 'terminal', 'pwsh'].includes(t));
 ...
 * @param {{ parent:unknown, prompt:string, label?:string, maxDepth?:number, provider?:object }} args
 */
...
export default { ROLES, ROLE_KEYS, roleSpec, phaseOfRole, subagentRequest, WRITE_TOOLS };
