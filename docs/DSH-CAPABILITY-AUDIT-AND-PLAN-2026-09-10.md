...
| **O16** | ✅ **第一批已收敛**（2026-09-12 · T22，剩余判定不硬收敛） | QUAL-2 shared-utils 复用：退避注册 4 插件用 `registerRouteWithRetry`；去重 2 插件用 `createDedupNotifier`；loopback 收敛 2 处逐字节等价 | `host-services/lib/shared-utils.js`（新增 `isLoopback`） | 重复代码行为漂移 | 剩余**不硬收敛**：`command-guard` 双路由**原子注册**（拆两次会改「整体重试→各自退避」语义）；`session-hygiene` 去重是 **bounded queue + 批量聚合**（与 `createDedupNotifier` 无界 Map 语义不同）；第二三批（log/原子写/SSRF）非第一批、低收益/高危 | 低 |
...
| W4 性能与结构 | 🟡 进行中（O5 已结 2026-09-12 · T18；O16 第一批已结 2026-09-12 · T22） | ✅ O5 vision-engine 路径配置化 + 退出钩子（默认行为零变化；契约测试 9/9）· ⬜ O3 会话异步解码 · ⬜ O12 routing-suite 拆分 · ✅ O16 第一批（退避+通知+loopback）已收敛（剩余项语义有差异，见 O16 行） |
