...
- 竞态修复：`dsh-self-maintenance` 状态路由退避重试**已实测生效**（19:27 重启约 2s 注册成功）；`dsh-session-hygiene` 同款修复**代码已入库、待下次重启验证**。
- git：`master @ 79386a1`（WORKTREE CLEAN，**未推送**；本次清扫全部改动在此提交）。

## 2. 待办门（均需用户动作）

1. 下次重启后：搜 `%APPDATA%\DSH Desktop\logs\dsh-*.log` 有无 `[session-hygiene] report route registered`，并重跑 `verify-features.ps1` 确认 51/51。
2. `git push`（可选，默认不推；本机桌面运行无需远端）。
